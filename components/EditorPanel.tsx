...
import { convertMarkdownToHtml, convertHtmlToMarkdown, generateDocBlob } from '../utils/formatHelpers';
import { parseFile } from '../utils/fileHelpers';
import { 
  IconCheck, IconX, IconBold, IconItalic, IconH1, IconH2, IconList, 
  IconUpload, IconDownload, IconEdit, IconEye, IconPlusChat
} from './Icons';
...
interface EditorPanelProps {
  ...
  isDiffMode: boolean; 
  highlights?: string[]; // New prop for analysis highlights
  onAcceptChange: () => void;
  onRejectChange: () => void;
...
const EditorPanel: React.FC<EditorPanelProps> = ({
  ...
  const backdropRef = useRef<HTMLDivElement>(null); // Ref for highlighting backdrop
  ...
  const diffChanges: DiffChange[] = useMemo(() => {
    ...
    return [];
  }, [contractMarkdown, proposedMarkdown, isDiffMode]);
  ...
  // Detect Text Selection in Draft Mode
  const handleMouseUp = (e: React.MouseEvent<HTMLTextAreaElement>) => {
    ...
    if (start !== end) {
      ...
      // We use the mouse event client coordinates for simplicity
      // Ideally, we'd calculate caret position, but that's complex in a raw textarea
      const x = e.clientX;
      const y = e.clientY;
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert("Failed to import file. Please ensure it is a valid DOCX, PDF, MD, or TXT file.");
    } finally {
    ...
    highlights.forEach(highlight => {
      ...
      for (let i = 0; i < segments.length; i++) {
        if (!segments[i].isHighlight) {
          const parts = segments[i].text.split(highlight);
          if (parts.length > 1) {
            const newSegments = [];
            for (let j = 0; j < parts.length; j++) {
              newSegments.push({ text: parts[j], isHighlight: false });
              if (j < parts.length - 1) {
                newSegments.push({ text: highlight, isHighlight: true });
              }
            }
    ...
    <div className="flex flex-col h-full bg-zinc-950 relative overflow-hidden">
      ...
      <div className="h-14 border-b border-zinc-800 flex items-center justify-between bg-zinc-900 px-4 shrink-0 z-20">
        ...
        <div className="flex items-center gap-2">
             ...
             <button onClick={handleExport} className="p-2 text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800 rounded transition-colors">
      ...
      <div className="flex-1 overflow-hidden relative">
            ...
            {isDiffMode ? (
              ...
              <div className="flex-1 relative overflow-hidden">
                ...
                <div className="absolute bottom-4 right-4 flex gap-2 z-20 pointer-events-none">
                  <div className="px-3 py-1 rounded-full bg-zinc-900 border border-zinc-800 text-xs text-zinc-500">Markdown</div>
                </div>
              </div>
            )}
              ...
              <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex flex-col items-center gap-2 z-50">
                <div className="flex items-center gap-3 bg-zinc-900 p-3 rounded-xl shadow-2xl border border-zinc-700/80 backdrop-blur-xl animate-in slide-in-from-bottom-8">
                  <div className="flex flex-col px-2 border-r border-zinc-700">
                      <span className="text-xs font-semibold text-zinc-100">Review Changes</span>
                  </div>
        ...
        {/* FORMAT MODE */}
        {activeTab === 'format' && (
          <div className="absolute inset-0 overflow-y-auto bg-zinc-100/90 flex justify-center p-8">
            <div className="w-full max-w-[210mm] min-h-[297mm] h-auto bg-white shadow-xl my-4 ring-1 ring-black/5">
              <div 
                ref={richTextRef}
                contentEditable
                onBlur={handleRichTextBlur}
                className="editor-content px-12 py-16 text-zinc-900 font-serif"
